...
// --- Mensajes automáticos y limpieza ---
let mensajesGuardados = []
const sendMessage = async (ctx, texto, extra={}) => {
  const mensaje = await ctx.reply(texto, extra)
  mensajesGuardados.push(mensaje.message_id)
  if (mensajesGuardados.length > 4) {
    const msgId = mensajesGuardados.shift()
    try { await ctx.deleteMessage(msgId) } catch(e) {}
  }
  return mensaje
}

// --- Función helper para teclados con botón regresar ---
const buildKeyboard = (buttons) => {
  return { inline_keyboard: [...buttons, [{ text: "⬅️ Regresar", callback_data: "menu_principal" }]] }
}
...
bot.start(async (ctx) => {
  ...
  if (!users[id]) {
    ...
    saveDB()
    await sendMessage(ctx, `VERIFICACIÓN\n\nEscribe este código:\n\`${code}\``, { parse_mode: "Markdown" })
...
bot.on('text', async (ctx) => {
  ...
  if (ctx.session?.waitingWallet) {
    if (!/^0x[a-fA-F0-9]{40}$/.test(text)) return sendMessage(ctx, "Wallet inválida")
    const amount = users[id].balance
    if (amount < 1) return sendMessage(ctx, "Mínimo 1 USDT")
    await sendMessage(ctx, "Procesando retiro…")
    try {
      const tx = await USDT.transfer(text, ethers.parseUnits(amount.toFixed(6), 18))
      await tx.wait()
      users[id].balance = 0
      saveDB()
      await sendMessage(ctx, `Retiro exitoso (${amount.toFixed(3)} USDT)\nhttps://bscscan.com/tx/${tx.hash}`)
    } catch (e) {
      await sendMessage(ctx, "Error en retiro: " + (e.reason || e.message).slice(0,100))
    }
...
bot.action('menu_mineria', async (ctx) => {
  ...
  await ctx.answerCbQuery()
  if (!u || u.refValidos < 5) return sendMessage(ctx, `Necesitas 5 referidos activos\nTienes: ${u?.refValidos || 0}/5`)
  ...
  ctx.reply(
    puede ? "*¡RECLAMA 0.03 USDT!*" : "Espera 24h",
    {
      parse_mode: "Markdown",
      reply_markup: puede
        ? buildKeyboard([[{ text: "RECLAMAR", callback_data: "mine" }]])
        : buildKeyboard([])
    }
  )
})
...
bot.action('menu_ganancias', async (ctx) => {
  await ctx.answerCbQuery()
  const u = users[ctx.from.id]
  await sendMessage(ctx, `Balance: ${(u?.balance || 0).toFixed(3)} USDT`)
  mainMenu(ctx)
})
...
bot.action('menu_retiro', async (ctx) => {
  await ctx.answerCbQuery()
  const u = users[ctx.from.id]
  if (!u || u.balance < 1) return sendMessage(ctx, "Mínimo 1 USDT")
  sendMessage(ctx, "Envía tu wallet BEP-20:")
...
bot.action('menu_referidos', async (ctx) => {
  ...
  await sendMessage(ctx, `Referidos totales: ${u?.referidos || 0}\nActivos: ${u?.refValidos || 0}\n\nLink de invitación:\n${link}`)
})
...
bot.action(/plan(\d+)/, async (ctx) => {
  ...
  await ctx.answerCbQuery()
  await sendMessage(ctx, `Envía exactamente ${monto}.000000 USDT a:\n${BOT_WALLET}\nPago detectado automáticamente.`)
})
