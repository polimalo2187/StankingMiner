...
// === COMANDOS ===
bot.hears('Ganancias', ctx => ctx.reply(`Balance: ${users[ctx.from.id.toString()].balance.toFixed(3)} USDT`))
...
bot.hears('Referidos', async ctx => {
  ...
  ctx.replyWithMarkdown(`*Referidos*\nTotal: \( {users[id].referidos}\nVálidos: \){users[id].ref_validos}\n\nEnlace:\n${link}`)
})
...
// === ARRANQUE DEFINITIVO (FIX 409 PARA SIEMPRE) ===
bot.launch({
  dropPendingUpdates: true   // ← ESTO MATA CUALQUIER CONFLICTO 409
}).then(() => {
