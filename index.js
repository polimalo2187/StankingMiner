...
bot.start(async (ctx) => {
  ...
  if (!users[id]) {
    ...
    await ctx.reply(`Escribe tu código de verificación:\n\n\`${users[id].verifyCode}\``, { parse_mode: "Markdown" })
  } else {
    menu(ctx)
  }
})

// === CÓDIGO + RETIRO + COMANDOS ===
bot.on('text', async (ctx) => {
  ...
  if (ctx.session?.step === "retiro") {
    ...
    try {
      ...
      await ctx.reply("Error en retiro: " + e.message.slice(0,150))
    }
...
// === BOTONES ===
bot.hears('Ganancias', ctx => ctx.reply(`Tu balance: ${users[ctx.from.id.toString()].balance.toFixed(3)} USDT`))
...
bot.action(/buy_(\d+)/, async (ctx) => {
  ...
  await ctx.replyWithMarkdown(`*Plan \( {monto} USDT*\nEnvía * \){monto} USDT* a esta wallet:\n\`${BOT_WALLET}\`\nLuego toca CONFIRMAR`, 
...
bot.hears('Minería', ctx => {
  const id = ctx.from.id.toString()
  const u = users[id]
  if (u.ref_validos < 5) return ctx.reply(`Necesitas 5 referidos con plan\nTienes: ${u.ref_validos}/5`)
  const puede = Date.now() - (u.last_mine || 0) >= 86400000
  ctx.reply(puede ? "¡Puedes reclamar 0.03 USDT!" : "Espera 24h", puede ? Markup.inlineKeyboard([[Markup.button.callback("RECLAMAR 0.03 USDT", "mine")]]) : null)
})
...
bot.launch()
console.log('STANKING MINER 100% ACTIVO, VERIFICADO Y COBRANDO 24/7 – TODO PERFECTO')
