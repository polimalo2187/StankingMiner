...
bot.start(async (ctx) => {
  ...
  if (!users[id]) {
    ...
    users[id] = { verified: false, verifyCode: code, balance: 0, planes: [], ref_validos: 0, last_mine0, referidos: 0, inviter: ref }
    save()
    await ctx.reply(`VERIFICACIÓN REQUERIDA\n\nEscribe este código para continuar:\n\n\`${code}\``, { parse_mode: "Markdown" })
    if (ref && users[ref]) {
...
bot.on('text', async (ctx) => {
  ...
  if (ctx.session?.step === "retiro") {
    if (!text.match(/^0x[a-fA-F0-9]{40}$/)) return ctx.reply("Wallet inválida")
...
bot.action(/confirm_(\d+)/, async (ctx) => {
  ...
  save()
  await ctx.reply("PLAN ACTIVADO! Reclama en 24h")
  menu(ctx)
})
...
bot.launch()
console.log('STANKING MINER CON VERIFICACIÓN ACTIVADA – 100% SEGURO Y FUNCIONANDO 24/7')
