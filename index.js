...
console.log('STANKING MINER - INICIANDO 100% AUTOMÁTICO...')
...
// === WALLET CORRECTA PARA ETHERS V6 ===
...
bot.start(async (ctx) => {
  ...
  if (!users[id]) {
    ...
    users[id] = {
      balance: 0, planes: [], referidos: 0, refValidos: 0,
      lastMine: 0, verified: false, code, inviter: ref
    }
    saveDB()
...
bot.hears('Minería', ctx => {
  ...
  ctx.reply(puede ? "*¡RECLAMA 0.03 USDT!*" : "Espera 24h", {
    parse_mode: "Markdown",
    reply_markup: puede ? {inline_keyboard: [[{text: "RECLAMAR", callback_data: "mine"}]]} : null
  })
})
...
// === DETECTOR DE PAGOS AUTOMÁTICO (cada 10 seg) ===
let lastBlock = 0
setInterval(async () => {
  try {
...
bot.launch({ dropPendingUpdates: true })
  .then(() => {
    bot.telegram.getMe().then(i => {
      console.log(`BOT 100% AUTOMÁTICO ENCENDIDO: @${i.username}`)
